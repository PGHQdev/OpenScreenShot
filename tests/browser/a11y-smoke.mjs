...
async function popupFirstRun(browser, base, messages) {
  ...
  step('POPUP — first run (pin nudge + the permission-ask trust line)');
  ...
  // Chrome caps a popup at 600px and scrolls past it. First run is the
  // tallest the main view ever is — the pin nudge and the trust line both
  // render, and neither does once the extension is pinned and granted — so
  // it is the state the cap has to hold for. Measured at a 600px viewport,
  // where scrollHeight is the viewport's own height until the content
  // exceeds it.
  const firstRunHeight = await page.evaluate(() => ({
    scrollHeight: document.documentElement.scrollHeight,
    content: Math.ceil(document.querySelector('.app').getBoundingClientRect().bottom),
  }));
  assert(
    firstRunHeight.scrollHeight <= 600,
    `the first-run popup needs no scroll at Chrome's 600px cap (${firstRunHeight.content}px of content)`,
  );
...
async function popupMain(browser, base, messages) {
  ...
  step('POPUP — main surface (mode cards + record + footer)');
  await page.goto(`${base}/src/popup/index.html`, { waitUntil: 'networkidle0' });
