...
async function testPopup(browser, base, messages) {
  ...
  // Every segmented control now lives in Settings — the main view kept the
  // cards, the chips and the footer — so the seg assertions below open it.
  // The chips stay mounted behind it and are re-read on the way back.
  ...
  await emulateMedia(cdp, []);
  // Back to the chips, which are on the main view, for the same pair.
  await page.click('.icon-btn[aria-label="Back to capture modes"]');
  await page.waitForSelector('.chip-row');
  await emulateMedia(cdp, [{ name: 'forced-colors', value: 'active' }]);
  await new Promise((r) => setTimeout(r, 400));
  ...
  await page.evaluate((label) => {
    ...
    [...row.querySelectorAll('.seg-btn')].find((b) => b.textContent === 'JPEG').click();
  }, messages.settingsDefaultFormat.message);
  ...
  // falling back to a detached probe. `.seg-btn` and `.token-chip` only
  // render in Settings now, so those two take the probe path.
  await page.click('.icon-btn[aria-label="Back to capture modes"]');
  await page.waitForSelector('.chip-row');
