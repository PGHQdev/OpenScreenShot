...
const out = path.join(path.dirname(fileURLToPath(import.meta.url)), 'pdf-corrected');
...
for (const name of ['openscreenshot', 'mdn']) {
  const [id, url, category] = pages.find((p) => p[0] === name);
  jobs.push({ id: `${id}-pdf-corrected`, url, category, mode: 'pdf', round: 1, endpoint: 'pdf' });
}
const results = [];
let lastStart = 0,
  browserTotal = 0;
...
for (const job of jobs) {
  if (browserTotal > 480000) {
    console.log('Browser-time safety cap reached');
    break;
  }
  ...
  try {
    const res = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${account}/browser-rendering/${job.endpoint}`,
      {
        method: 'POST',
    ...
    Object.assign(row, {
      ...
      browserMs: res.headers.has('x-browser-ms-used')
        ? Number(res.headers.get('x-browser-ms-used'))
        : null,
      bytes: buf.length,
    });
    const signature =
    ...
    if (row.artifactValid) {
      row.file = `artifacts/${job.id}.${job.endpoint === 'pdf' ? 'pdf' : 'png'}`;
      await fs.writeFile(path.join(out, row.file), buf);
      if (job.endpoint === 'screenshot') {
        row.width = buf.readUInt32BE(16);
        row.height = buf.readUInt32BE(20);
      }
  ...
  console.log(
    JSON.stringify({
      id: row.id,
      status: row.status,
      valid: row.artifactValid,
      seconds: +(row.elapsedMs / 1000).toFixed(2),
      browserMs: row.browserMs,
      size: row.width ? `${row.width}x${row.height}` : undefined,
      error: row.error,
    }),
  );
  if ([401, 403, 429].includes(row.status)) {
    console.log('Stopping on auth/quota response');
    break;
  }
}
