...
const jobs=[];
for(let round=1;round<=2;round++) for(const name of ['screenshotone','astro']){
 const [id,url,category]=pages.find(p=>p[0]===name);
 jobs.push({id:`${id}-scroll-${round}`,url,category,mode:'desktop',round,endpoint:'screenshot'});
}
...
for(const job of jobs){
 ...
 const payload={url:job.url,viewport,scrollPage:true,gotoOptions:{waitUntil:'networkidle2',timeout:25000},...(job.endpoint==='screenshot'?{screenshotOptions:{fullPage:true,type:'png'}}:{pdfOptions:{format:'A4',printBackground:true}})};
