...
const quantile = (values, p) => {
  ...
  return a.length ? a[Math.ceil(p * a.length) - 1] : null;
};
const groups = {};
for (const mode of ['desktop', 'mobile', 'pdf']) {
  const all = rows.filter((r) => r.mode === mode),
    ok = all.filter((r) => r.artifactValid);
  groups[mode] = {
    attempts: all.length,
    validArtifacts: ok.length,
    p50Seconds: quantile(
      ok.map((r) => r.elapsedMs / 1000),
      0.5,
    ),
    p95Seconds: quantile(
      ok.map((r) => r.elapsedMs / 1000),
    ...
    browserSeconds: ok.reduce((n, r) => n + (r.browserMs ?? 0) / 1000, 0),
    missingUsageHeaders: all.filter((r) => r.browserMs == null).length,
  };
}
...
const summary = {
  ...
  quantileMethod:
    'nearest rank; valid artifacts only; includes transfer to local client; visual correctness assessed separately',
  groups,
  totalReportedBrowserSeconds: totalMs / 1000,
  reportedUsageMarginalBrowserCostUSD: (totalMs / 3600000) * 0.09,
  reportedUsageCostPer1000ValidArtifactsUSD:
    (((totalMs / 3600000) * 0.09) / rows.filter((r) => r.artifactValid).length) * 1000,
};
...
for (const r of rows) {
  ...
  if (r.endpoint === 'screenshot') {
    ...
    await image
      ...
      .toFile(path.join(dir, `previews/${r.id}.png`));
    await image
      .clone()
      .extract({ left: 0, top: 0, width: r.width, height: Math.min(1000, r.height) })
      .resize({ width: 720 })
      .toFile(path.join(dir, `previews/${r.id}-top.png`));
  }
